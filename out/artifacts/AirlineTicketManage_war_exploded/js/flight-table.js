...
table.GM({
    ...
    , ajax_url: 'http://127.0.0.1:8080/generateFlightData.action'
...
$(function () {
            ...
            success: function (data, status) {
                ...
                if (data.status == "0") {
                    ...
                    for (var i = 0; i < count; i++) {
                        res[i] = "航班编号：" + dataArray[i] + (data[dataArray[i]] == 0 ? "无法删除\n" : "删除成功\n");
                    }
                    var output = res.join("");
                    alert(output);
    ...
    $("#dialog-add .edit-infos .btn .submit-btn").click(function () {
        if ($("span.edit-warning").length != 0) {
            ...
            $.ajax({
                ...
                dataType: 'json',
                data: {
                    flight_id: $("#dialog-add .edit-infos").children(":eq(0)").find("input").val(),
                    dep_city: $("#dialog-add .edit-infos").children(":eq(1)").find("input").val(),
                    arr_city: $("#dialog-add .edit-infos").children(":eq(2)").find("input").val(),
                    flight_date: $("#dialog-add .edit-infos").children(":eq(3)").find("input").val(),
                    dep_time: $("#dialog-add .edit-infos").children(":eq(4)").find("input").val(),
                    arr_time: $("#dialog-add .edit-infos ").children(":eq(5)").find("input").val(),
                },
                success: function (data, status) {
                    if (data.status == "1") {
                        alert("添加成功！");
    ...
    $("table").delegate('.edit-table', 'click', function () {
        ...
        $("#dialog-edit .edit-infos").children(":eq(4)").find("input").val(formatDate($(this).parents("tr").children(":eq(6)").text()));
        $("#dialog-edit .edit-infos").children(":eq(5)").find("input").val(formatDate($(this).parents("tr").children(":eq(7)").text()));
    });
    ...
    $("#dialog-edit .edit-infos .btn .submit-btn").click(function () {
        if ($("span.edit-warning").length != 0) {
            ...
            $.ajax({
                ...
                dataType: 'json',
                data: {
                    flight_id: $("#dialog-edit .edit-infos").children(":eq(0)").find("input").val(),
                    dep_city: $("#dialog-edit .edit-infos").children(":eq(1)").find("input").val(),
                    arr_city: $("#dialog-edit .edit-infos").children(":eq(2)").find("input").val(),
                    flight_date: $("#dialog-edit .edit-infos").children(":eq(3)").find("input").val(),
                    dep_time: $("#dialog-edit .edit-infos").children(":eq(4)").find("input").val(),
                    arr_time: $("#dialog-edit .edit-infos ").children(":eq(5)").find("input").val(),
                },
                success: function (data, status) {
                    if (data.status == "1") {
                        alert("修改成功！");
