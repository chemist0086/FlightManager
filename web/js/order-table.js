...
table.GM({
    ...
    , ajax_url: 'http://127.0.0.1:8080/generateOrderData.action'
    ...
    $("#dialog-add .edit-infos .btn .submit-btn").click(function () {
        if ($("span.edit-warning").length != 0) {
            ...
            $.ajax({
                ...
                dataType: 'json',
                data: {
                    order_id: $("#dialog-add .edit-infos").children(":eq(0)").find("input").val(),
                    pass_id: $("#dialog-add .edit-infos").children(":eq(1)").find("input").val(),
                    pass_name: $("#dialog-add .edit-infos").children(":eq(2)").find("input").val(),
                    pass_idcard: $("#dialog-add .edit-infos").children(":eq(3)").find("input").val(),
                    pass_passport: $("#dialog-add .edit-infos").children(":eq(4)").find("input").val(),
                    flight_id: $("#dialog-add .edit-infos").children(":eq(5)").find("input").val(),
                    dep_city: $("#dialog-add .edit-infos ").children(":eq(6)").find("input").val(),
                    arr_city: $("#dialog-add .edit-infos").children(":eq(7)").find("input").val(),
                    dep_time: $("#dialog-add .edit-infos").children(":eq(8)").find("input").val(),
                    arr_time: $("#dialog-add .edit-infos").children(":eq(9)").find("input").val(),
                    deli_id: $("#dialog-add .edit-infos").children(":eq(10)").find("input").val(),
                    deli_name: $("#dialog-add .edit-infos").children(":eq(11)").find("input").val(),
                    price_purc: $("#dialog-add .edit-infos").children(":eq(12)").find("input").val(),
                },
                success: function (data, status) {
                    if (data.status == "1") {
                        alert("添加成功！");
    ...
    $("#dialog-edit .edit-infos .btn .submit-btn").click(function () {
        if ($("span.edit-warning").length != 0) {
            ...
            $.ajax({
                ...
                dataType: 'json',
                data: {
                    order_id: $("#dialog-edit .edit-infos").children(":eq(0)").find("input").val(),
                    price_purc: $("#dialog-edit .edit-infos").children(":eq(12)").find("input").val(),
                },
                success: function (data, status) {
                    if (data.status == "1") {
                        alert("修改成功！");
