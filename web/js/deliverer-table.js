...
$(function () {
    ...
    $("#dialog-add .edit-infos .btn .submit-btn").click(function () {
        if ($("span.edit-warning").length != 0) {
            ...
            $.ajax({
                ...
                dataType: 'json',
                data: {
                    deli_id: $("#dialog-add .edit-infos").children(":eq(0)").find("input").val(),
                    deli_name: $("#dialog-add .edit-infos").children(":eq(1)").find("input").val(),
                    deli_age: $("#dialog-add .edit-infos").children(":eq(2)").find("input").val(),
                    deli_sex: $("#dialog-add .edit-infos").children(":eq(3)").find("select").val(),
                    deli_phone: $("#dialog-add .edit-infos ").children(":eq(4)").find("input").val(),
                    deli_email: $("#dialog-add .edit-infos").children(":eq(5)").find("input").val()
                },
                success: function (data, status) {
                    if (data.status == "1") {
                        alert("添加成功！");
    ...
    $("#dialog-edit .edit-infos .btn .submit-btn").click(function () {
        if ($("span.edit-warning").length != 0) {
            ...
            console.log($("#dialog-add .edit-infos").children(":eq(0)").find("input"));
            $.ajax({
